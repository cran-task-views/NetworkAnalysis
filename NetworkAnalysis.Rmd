...
knitr::opts_chunk$set(echo=FALSE, message=FALSE, warning=FALSE, error=FALSE, results="asis")
options(width = 10000)
```
```{r}
bib <- rbibutils::readBib("REFERENCES.bib")
library(tools)

## A function that keeps track of a vector of distinct keys in the
## orders they first appeared. tocite(key) appends key to the list if
## not already present and returns the updated list
## invisibly. tocite() returns the list so far.
tocite <- local({
  keys <- character(0)
  function(key){
    if(missing(key)) keys
    else keys <<- unique(c(keys, key))
  }
})

## Preserve these directives for ctv.
for(name in c("doi", "view", "pkg", "bioc", "github", "rforge", "gcode", "ohat"))
  assign(name, function(...)sprintf('`r %s`', deparse1(match.call())))

cite <- function(key){
  ## Check if key exists.
  if(!hasName(bib, key)) stop("Key ", sQuote(key), " not found!")

  ## Add it to the list and look up its location.
  k <- match(key, tocite(key))
  sprintf("[\\[%0.0f\\]](#cite-%0.0f)", k, k)
}
```
...
- `r pkg("statnet", priority = "core")`, sometimes used only as `r pkg("sna",
  priority = "core")` and/or `r pkg("network", priority = "core")` `r cite("Bu08n")` is a more
...
- `r pkg("latentnet")` `r cite("KrHa08f")` provides functions to fit and simulate latent position
and cluster model using `r pkg("network")` `r cite("Bu08n")` objects and compatibly with
...
exponential-family random graph models (ERGM) `r cite("LuKo12e")`. Depending on specific needs,
...
cross-sectional or longitudinal observed partitions `r cite("HoBl23m")`, i.e. sets of
...
## References
```{r, bibstyle}
## A custom R bib style that generates markdown.

library(tools)

mdstyle <- bibstyle("md", as.environment(as.list(bibstyle("JSS"), all.names=TRUE)), .default = FALSE)
parent.env(mdstyle) <- parent.env(bibstyle())
for(name in names(mdstyle)){
  if(is.function(mdstyle[[name]]) &&
     identical(environment(mdstyle[[name]]), bibstyle("JSS")))
    environment(mdstyle[[name]]) <- mdstyle
}

mdstyle$fmtTitle <- function (title) 
if (length(title)) {
    paste0('"', addPeriod(collapse(cleanupLatex(title))), 
        '"')
}
environment(mdstyle$fmtTitle) <- mdstyle

mdstyle$volNum <- function (paper) {
    if (length(paper$volume)) {
        result <- paste0("**", collapse(paper$volume), "**")
...
environment(mdstyle$volNum) <- mdstyle

mdstyle$shortName <- function (person) 
{
    if (length(person$family)) {
        result <- cleanupLatex(person$family)
        if (length(person$given) == 1)
          paste(result, cleanupLatex(person$given))
        else if(length(person$given) > 1)
          paste(result, cleanupLatex(person$given[[1]]), paste0(sapply(person$given[-1], cleanupLatex), ".", collapse = " "))
        else result
    }
    else paste(cleanupLatex(person$given), collapse = " ")
}
environment(mdstyle$shortName) <- mdstyle

mdstyle$fmtDOI <- function(s) if(length(s)) paste(sprintf('`r doi("%s")`', s), collapse = " ")
environment(mdstyle$fmtDOI) <- mdstyle

mdstyle$extraInfo <- function(paper) {
  loc <- fmtDOI(paper$doi) %||% fmtEprint(paper$eprint) %||% fmtUrl(paper$url)
  
  result <- paste(c(loc, fmtNote(paper$note)), collapse = ", ")
  if (nzchar(result))
    result
}
environment(mdstyle$extraInfo) <- mdstyle

mdstyle$fmtISBN <- mdstyle$fmtISSN <- function(s) NULL

mdstyle$emph <- function(s) if(length(s)) paste0("*", s, "*")

```
```{r, print-bibliography}
l <- tocite()
i <- seq_along(l)

cat(paste0(sprintf("%0.0f. []{#cite-%0.0f}", i, i), bib[l] |> sapply(format, "text", .bibstyle = "md"), collapse = "\n\n"))
```
